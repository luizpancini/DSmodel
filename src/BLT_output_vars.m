function outputs = BLT_output_vars(t,x,y,xdot)
...
outputs.alpha_plunge = y(36,:);
outputs.R = outputs.qR; dummy = (outputs.R>1); outputs.R(dummy) = 1;

outputs.so_state = x(3,:);
outputs.theta = x(3,:)./outputs.so_lim;
outputs.f2prime_n = x(4,:);
outputs.f2prime_m = x(5,:);
outputs.f2prime_c = x(6,:);
outputs.RD = x(7,:);
outputs.RD_theta = x(8,:);
