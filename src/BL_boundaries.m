...
boundary(4)=(abs(so_i)-alpha1n_i)*(abs(so_ip1)-alpha1n_i);              % fprime_n: so_state = alpha1_n
boundary(5)=(abs(so_i)-alpha1m_i)*(abs(so_ip1)-alpha1m_i);              % fprime_m: so_state = alpha1_m
boundary(6)=(abs(so_i)-alpha1c_i)*(abs(so_ip1)-alpha1c_i);              % fprime_c: so_state = alpha1_c
boundary(7)=(abs(theta_ip1)-1)*(abs(theta_i)-1);                        % c_c: abs(theta) = 1
boundary(8)=(t_ip1-tv0)*(t_i-tv0);                                      % cn_v: first vortex begins
boundary(9)=(t_ip1-tv0-2*TvL)*(t_i-tv0-2*TvL);                          % cn_v: first vortex ends
boundary(10)=q_bar_ip1*q_bar_i;                                         % q_bar = 0 
